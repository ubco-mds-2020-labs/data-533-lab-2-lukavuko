...
def min_downpayment(price):
    ...
    if  price < 0 or (type(price) != float and type(price) != int):
        print("Invalid price")
        return None
    elif price < 500000:
        return price*0.05
...
def mort_rate(term):
    ...
    if term < 1:
        return 'Not a viable term length; must be at least 1.'
    ...
    x = term
    return (0.0167*x**2 - 0.0337*x + 1.6851)
...
def mortgage_insurance(price, downpayment):
    ...
    if DP_proportion >= 0.2:
        ...
        return 'Downpayment must be at least 5% the asset value'
...
def monthly_payment(principal, mortgage_rate, amortization, months = False):
    ...
    R = (MR/100/12 + 1)   ## monthly interest rate
...
def total_interest(principal, mortgage_rate, monthly_payment):
    ...
    R = annual_interest/1200   ## monthly interest rate
    CumInterest = 0
